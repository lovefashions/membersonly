...
function validateImageSrc(openingNode) {
  if (
    !openingNode ||
    !openingNode.name ||
    (openingNode.name.name !== 'img' && openingNode.name.property?.name !== 'img')
  ) {
                ...
                for (const child of elementNode.children) {
                  ...
                  if (t.isJSXElement(child)) {
                    ...
                    if (childNode.selfClosing) {
                      ...
                      if (
                        !/^[A-Z]/.test(childName) &&
                        !checkTagNameEditable(childNode, EDITABLE_JSX_TAGS)
                      ) {
        ...
        req.on('end', async () => {
          ...
          try {
            ...
            if (!parsedId) {
              res.writeHead(400, { 'Content-Type': 'application/json' });
              return res.end(
                JSON.stringify({ error: 'Invalid editId format (filePath:line:column)' })
              );
            }
            ...
            if (!targetNodePath) {
              ...
              return res.end(
                JSON.stringify({ error: 'Target node not found by line/column', editId })
              );
            }
            ...
            const isImageElement =
              targetOpeningElement.name && targetOpeningElement.name.name === 'img';
