...
function extractHelmetData(content, filePath, routes) {
  ...
  const url =
    routes.length && routes.has(fileName) ? routes.get(fileName) : generateFallbackUrl(fileName);
...
function main() {
  ...
  if (!fs.existsSync(pagesDir)) {
    ...
    pages = reactFiles.map((filePath) => processPageFile(filePath, routes)).filter(Boolean);
  }
